...
import static org.jala.university.infrastructure.external.ExternalServiceAPI.*;
...
public final class UserHomeController extends BaseController {
    ...
    private HBox createSearchResultItem(ExternalServiceDto service) {
        ...
        boolean isLinked = linkService.isServiceLinked(getDefaultCustomerId(), service.getId());
    ...
    private void performLinkAction(UUID serviceId) {
        new Thread(() -> {
            try {
                linkService.linkService(getDefaultCustomerId(), serviceId);
    ...
    private void loadLinkedServices() {
        new Thread(() -> {
            try {
                List<ExternalServiceDto> linked = linkService.getLinkedServices(getDefaultCustomerId());
    ...
    private void performUnlinkAction(UUID serviceId) {
        new Thread(() -> {
            try {
                linkService.unlinkService(getDefaultCustomerId(), serviceId);
